...
Future<Stream<OrderStatus>> getOrderStatus() async {
  User _user = userRepo.currentUser.value;
  if (_user.apiToken == null) {
    return Stream.value(OrderStatus());
  }
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url =
...
Future<Order> addOrder(Order order, Payment? payment) async {
  ...
  if (response.statusCode == 200) {
    ...
    throw Exception("Unauthorized: API token might be invalid or expired");
  } else if (response.statusCode == 500) {
    throw Exception("Server Error: Something went wrong on the backend");
  } else {
