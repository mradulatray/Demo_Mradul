...
// some changes
  ...
  // New Price Changes
  static Widget getDiscountedPrice(
      String payment, double myPrice, BuildContext context,
            ...
            ? TextSpan(
                ...
                style: style == null
                    ...
                    : style.merge(TextStyle(
                        fontWeight: FontWeight.w400, fontSize: style.fontSize)),
                children: <TextSpan>[
                  TextSpan(
                      text: myPrice.toStringAsFixed(
                              setting.value.currencyDecimalDigits ?? 0) ??
                          '',
                      style: style ?? Theme.of(context).textTheme.titleMedium),
                ],
              )
    ...
    try {
      ...
      return RichText(
        ...
        text: setting.value.currencyRight != null &&
            ...
            : TextSpan(
                text: myPrice
                    .toStringAsFixed(setting.value.currencyDecimalDigits ?? 0),
                style: style ?? Theme.of(context).textTheme.titleMedium,
                children: <TextSpan>[
                  TextSpan(
                    text: setting.value.defaultCurrency,
  ...
  static Widget getPriceNew(double myPrice, BuildContext context,
      {TextStyle? style, String zeroPlaceholder = '-'}) {
    if (style != null) {
      style = style.merge(TextStyle(fontSize: (style.fontSize ?? 0) + 2));
    }
    try {
      //debugPrint(";pp"+myPrice.toString());
      /*  if (myPrice == 0) {
        // return Text(zeroPlaceholder, style: style ?? Theme.of(context).textTheme.subtitle1);
        //   TODO CHANGED BY ARNAB 2/12/24
        return RichText(
          softWrap: false,
          overflow: TextOverflow.fade,
          maxLines: 1,
  ...
  static double getTotalOrderPrice(FoodOrder foodOrder) {
    double total = foodOrder.price ?? 0;
    foodOrder.extras?.forEach((extra) {
      total = total + (extra.price ?? 0);
    });
    total *= (foodOrder.quantity ?? 0);
    return total;
  }
  ...
  static double getTotalOrdersPrice(Order order) {
    ...
    total = total + (order.deliveryFee ?? 0);
  ...
  // Future<void> getDataOfPayment() async {
  ...
  static bool canDeliveryNew(Restaurant restaurant, String deliveryType,
      {List<Cart>? carts}) {
    bool _can = false;
    if (deliveryType == "NO_DELIVERY_FEE") {
      _can = false;
    } else {
      _can = true;
    }
