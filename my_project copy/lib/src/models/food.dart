...
class Food {
  ...
  Food.fromJSON(Map<String, dynamic> jsonMap) {
    try {
      ...
      foodReviews = [];
      nutritions = [];
      print(CustomTrace(StackTrace.current, message: e as String));
    }
  }
  ...
  double getRate() {
    ...
    _rate = _rate > 0 ? (_rate / (foodReviews?.length ?? 1)) : 0;
    return _rate;
  }
  ...
  Coupon _couponDiscountPrice(Coupon coupon) {
    ...
    if (coupon.discountType == 'fixed') {
      price = (price ?? 0) - (coupon.discount ?? 0);
    } else {
      price = (price ?? 0) - ((price ?? 0) * (coupon.discount ?? 0) / 100);
    }
